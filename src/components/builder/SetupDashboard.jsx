        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <section className="space-y-6 bg-white p-8 rounded-[2rem] shadow-sm border border-gold/5">
            <h2 className="flex items-center gap-3 text-lg font-serif text-deep-green border-b border-gold/10 pb-4">
              <Heart className="w-5 h-5 text-gold" /> The Happy Couple & Language
            </h2>
            <div className="space-y-4">
              <div className="p-4 bg-gold/5 rounded-2xl border border-gold/10 mb-4">
                <label className="block text-[10px] uppercase tracking-widest text-gold font-bold mb-2">Default Invitation Language</label>
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all border ${
                        config.language === lang 
                          ...
                          : 'bg-white text-neutral-400 border-gold/20 hover:border-gold/40'
                      }`}
                    >
                      {lang === 'en' ? 'English' : 'Bengali (WB)'}
                    </button>
                  ))}
                </div>
                <p className="text-[8px] text-neutral-400 mt-2 italic">* This sets the language for all guests by default.</p>
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex items-end">
                <div className="w-full md:w-auto">
                  <label className="block text-[10px] uppercase tracking-widest text-neutral-400 mb-2 font-bold ml-1">Guest Language</label>
                  <div className="flex gap-2">
                    <select
                      id="guest-lang-select"
                      className="p-4 bg-neutral-50 border border-neutral-100 rounded-2xl focus:ring-2 focus:ring-gold/20 outline-none transition-all font-serif text-xs"
                      defaultValue={config.language}
                    >
                      <option value="en">English</option>
                      <option value="bn">Bengali (WB)</option>
                    </select>
                    <button
                      onClick={() => {
                        const guestName = document.getElementById('guest-name-input').value;
                        const guestLang = document.getElementById('guest-lang-select').value;
                        if (!guestName) {
                          alert("Please enter a guest name first.");
                          return;
                        }
                        const baseUrl = window.location.origin;
                        const finalLink = `${baseUrl}/?id=${config.weddingId}&mode=guest&guest=${encodeURIComponent(guestName)}&lang=${guestLang}`;
                        navigator.clipboard.writeText(finalLink);
                        alert(`Personalized link for "${guestName}" in ${guestLang === 'en' ? 'English' : 'Bengali'} copied to clipboard!`);
                      }}
                      className="px-8 py-4 bg-gold text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:scale-105 transition-all flex items-center gap-2 justify-center"
                    >
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center bg-white/5 p-3 rounded-2xl border border-white/10 backdrop-blur-sm">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <GuestLinkShareIcons
                      ...
                      coupleLabel={`${config.couple.name1} & ${config.couple.name2}`}
                    />
                    <button
                      type="button"
                      onClick={() => {
                        ...
                        alert(`Guest Link Copied for ID: ${config.weddingId}!`);
                      }}
